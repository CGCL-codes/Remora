...
func (n *Node) broadcastPrepareMsg(Prepare PrepareMsg) {
	err := n.broadcast(PrepareMsgTag, Prepare)
	if err != nil {
		panic(err)
	}
}

func (n *Node) broadcastHelpMsg(HelpM HelpMsg) {
	err := n.broadcast(HelpMsgTag, HelpM)
	if err != nil {
		panic(err)
	}
