...
func New(name string, maxPool int, clusterAddr map[string]string, clusterPort map[string]int,
	...
	tsPublicKey *share.PubPoly, tsPrivateKey *share.PriShare, logLevel int, isFaulty bool, batchSize int, round int) *Config {
	return &Config{
		Name:                 name,
		MaxPool:              maxPool,
...
func LoadConfig(configPrefix, configName string) (*Config, error) {
	...
	conf := &Config{
		...
		TimeCon:      viperConfig.GetInt("timer_config"),
	}
