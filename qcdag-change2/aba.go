...
// Agreement.
func (b *ABA) inputValue(h int, payLoadHashes [][HASHSIZE]byte, cId uint8, dealer int) error {
	b.lock.Lock()
	defer b.lock.Unlock()
	b.aLogger.Debug("!!!!!!!!!!!!!!!!!!!! ABA is launched !!!!!!!!!!!!!!!!!!!!", "replica", b.node.Name,
		"height", h)
...
func (b *ABA) handleBvalRequest(msg *ABABvalRequestMsg) error {
	...
	if lenBval == 2*b.node.F+1 {
		...
		b.tryOutputAgreement(msg.Height, msg.PayLoadHashes, msg.Dealer)
	}
	return nil
}
...
func (b *ABA) tryOutputAgreement(height int, payLoadHashes [][HASHSIZE]byte, dealer int) {
